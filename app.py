...
def check_emggen_coverage(pms_live: pd.DataFrame, eg_final: pd.DataFrame, machinery_name: str, machinery_short: str):
    ...
    pms_live[has_col] = pms_live["Vessel_norm"].apply(lambda x: x in eg_vessels).astype(bool)

    mask_missing_eg = ~pms_live[has_col].fillna(False).astype(bool)
    missing_eg = pms_live.loc[
        mask_missing_eg,
        ["Fleet", "Group Owner", "Vessel Name", "JiBe Go-Live Date"]
    ].copy()
    ...
    summary = details.groupby(
        ["Fleet", "Group Owner", "Vessel Name", "JiBe Go-Live Date", "PMS"],
        dropna=False
    ).agg(
    ...
    summary[has_col] = summary[count_col].gt(0).astype(bool)
...
def check_job_code(pms_live: pd.DataFrame, job_file, job_code: str):
    ...
    check["Job_Linked"] = check["Job_Linked"].fillna(0).astype(int)
    check["Critical_Flag"] = check["Critical_Flag"].fillna(False).astype(bool)
    check[f"Job {job_code} Linked"] = check["Job_Linked"].gt(0).astype(bool)

    mask_missing_job = ~check[f"Job {job_code} Linked"].fillna(False).astype(bool)
    missing_job = check.loc[
        mask_missing_job,
        ["Fleet", "Group Owner", "Vessel Name", "JiBe Go-Live Date", "PMS"]
    ].copy()
...
def check_eform_availability(pms_live: pd.DataFrame, job_file):
    ...
    job["Has_Eform"] = job[eform_col].apply(has_eform).astype(bool)
    ...
    eform_check["Eform_Available"] = eform_check["Eform_Available"].fillna(False).astype(bool)

    eform_available = eform_check.loc[
        eform_check["Eform_Available"].fillna(False).astype(bool),
        ["Fleet", "Group Owner", "Vessel Name", "JiBe Go-Live Date", "PMS", "Eform_Links"]
    ].copy()

    mask_eform_missing = ~eform_check["Eform_Available"].fillna(False).astype(bool)
    eform_missing = eform_check.loc[
        mask_eform_missing,
        ["Fleet", "Group Owner", "Vessel Name", "JiBe Go-Live Date", "PMS"]
    ].copy()
    ...
    if st.button("🔄 Process Files", type="primary", use_container_width=True):
        try:
            with st.spinner("Processing files..."):
                ...
                machinery_name, machinery_short = detect_machinery_type(
                    machinery_file,
                    machinery_file.name if hasattr(machinery_file, 'name') else None
                )

                machinery_file.seek(0)
                eg_final = build_emggen_final(machinery_file)

                eg_summary, eg_details, eg_missing, count_col = check_emggen_coverage(
                    pms_live, eg_final, machinery_name, machinery_short
                )

                job_file.seek(0)
                detected_job_code = detect_job_code(
                    job_file,
                    job_file.name if hasattr(job_file, 'name') else None
                )
...
if st.session_state.get('processed', False):
    ...
    if job_check is not None and 'Frequency' in job_check.columns:
        ...
        combined_freqs = []
        for _, row in job_check.iterrows():
